...
<?php else: $rawJson = (string)($detailRow['response_json'] ?? ''); if($rawJson===''){ $rawJson = json_encode($detailJson, JSON_UNESCAPED_UNICODE); } ?><div class="detail-card"><div class="detail-card-header"><div class="detail-card-title">入力文</div></div><div class="detail-card-body"><?= nl2br(h((string)($detailRow['input_text'] ?? ''))) ?></div></div>
...
<?php if(!empty($detailRow['share_token'])): ?><div class="detail-card"><div class="detail-card-header"><div class="detail-card-title">共有プレビュー</div></div><iframe class="preview-frame" src="index.php?share=<?= h((string)$detailRow['share_token']) ?>"></iframe><div class="json-actions"><a target="_blank" href="index.php?share=<?= h((string)$detailRow['share_token']) ?>">新規タブで開く</a></div></div><?php endif; ?>
<?php endif; ?></div>
