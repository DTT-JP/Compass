...
window.addEventListener('scroll', () => {
  if (!scrollTicking) {
    requestAnimationFrame(() => {
      ...
      const tabWrap = $('sticky-tab-wrap');

      if (currentY > lastScrollY && currentY > 60) {
        // scrolling down: hide header
        header && header.classList.add('header-hidden');
        tabWrap && tabWrap.classList.add('tab-hidden');
      } else {
...
/* ─── タブ切り替え ─── */
// Mobile tabs (sticky bar)
const tabLineEl = $('tab-line');
const tabSitEl = $('tab-sit');
const tabHistEl = $('tab-hist');
if (tabLineEl) tabLineEl.onclick = () => switchTab('line');
if (tabSitEl) tabSitEl.onclick  = () => switchTab('sit');
if (tabHistEl) tabHistEl.onclick = () => switchTab('hist');

// Desktop tabs (inside left column)
const tabLineDEl = $('tab-line-d');
const tabSitDEl = $('tab-sit-d');
const tabHistDEl = $('tab-hist-d');
if (tabLineDEl) tabLineDEl.onclick = () => switchTab('line');
if (tabSitDEl) tabSitDEl.onclick  = () => switchTab('sit');
if (tabHistDEl) tabHistDEl.onclick = () => switchTab('hist');
...
/* ─── Report header bar ─── */
function showReportHeader(source) {
  const bar = $('report-header-bar');
  if (!bar) return;
  bar.classList.remove('hidden');
  const badge = $('report-source-badge');
...
function initChips(groupId, stateKey, colorClass) {
  ...
  grp.querySelectorAll('.chip').forEach(c => {
    ...
    c.onclick = () => {
      ...
      if (stateKey === 'partner') localStorage.setItem('c_partner', state.partner);
    };
  });
}
...
function initMultiChips(groupId, stateKey, colorClass) {
  ...
  const updateUI = () => {
    grp.querySelectorAll('.chip').forEach(c => {
      ...
      c.className = state[stateKey].includes(val) ? 'chip ' + colorClass : 'chip';
    });
  };
  ...
  grp.querySelectorAll('.chip').forEach(c => {
    c.onclick = () => {
      const val = c.dataset.val;
      if (val === 'わからない') {
        state[stateKey] = state[stateKey].includes('わからない') ? [] : ['わからない'];
      } else {
      ...
      if (state[stateKey].length === 0) state[stateKey] = ['わからない'];
      updateUI();
    };
  });
...
async function doSubmit() {
  ...
  const submitWrapSit = $('submit-wrap-sit');
  if (submitWrapSit) submitWrapSit.classList.add('hidden');
  $('result-panel').classList.add('hidden');
  const summaryPanel = $('input-summary-panel');
  if (summaryPanel) summaryPanel.classList.add('hidden');
  ...
  ['tab-line-d', 'tab-sit-d', 'tab-hist-d'].forEach(id => {
    const el = $(id);
    if (el) el.classList.remove('active');
  });
  hideReportHeader();
...
const btnSubmitSit = $('btn-submit-sit');
if (btnSubmitSit) btnSubmitSit.onclick = doSubmit;
...
function demoResult(text) {
  ...
  if (hi && !lo) return {
    ...
    psychology: `${p}はあなたのことをかなり気にしているよ！積極的に話しかけたり、あなたを気にかける言動がたくさん見られる。このまま自然に仲を深めていける感じがするね！`,
    advice: `${p}の好意に素直に反応してOK♪「一緒にいると楽しい」ってことを自然に伝えてみよう。`,
    ...
    matrix: { x: 65, y: 50 },
    matrixInterpretation: '高揚・追いかけモード！あなたに会いたい気持ちが強くて、積極的に動きたい状態みたい。',
    lang: { selfDisclosure: 62, mirroring: 71, pronounCount: 7, emojiSync: 68 },
    langInterpretation: '自分のことをよく話してくれてる＝心を開いてるサイン。',
    approaches: [
      { law: 'ゲイン・ロス効果', title: 'たまにそっけなくしてドキドキさせる', body: 'たまに短い返信にしてみると気にしてくれて追いかけてくれる気持ちが生まれるよ。', example: '「今ちょっと忙しい〜またね！」って短く切り上げてみると効果的かも' },
      { law: 'ツァイガルニク効果', title: '話を途中で切って「続き気になる」状態を作る', body: '話を完結させずに「続きは今度！」ってするだけで、相手の頭の中にあなたが残り続けるんだ。', example: '「実はちょっとびっくりすることがあって〜長くなるから今度話す！笑」' }
    ],
    usedModel: 'Demo'
  };
  ...
  if (lo) return {
    ...
    psychology: `${p}は今、部活や勉強で忙しいか、気持ちを整理中かもしれないよ。`,
    advice: `焦らずに数日こちらからの連絡をちょっとお休みしてみよう。`,
    ...
    matrix: { x: -20, y: -55 },
    matrixInterpretation: '停滞・お疲れモード。恋愛に使えるエネルギーが少ない状態みたい。',
    lang: { selfDisclosure: 18, mirroring: 25, pronounCount: 1, emojiSync: 15 },
    langInterpretation: '自分のことをあまり話してくれてない。少し距離を感じる状態かな。',
    approaches: [
      { law: 'アンダードッグ効果', title: '少し弱いところを見せて守りたいと思わせる', body: 'ちょっとした困りごとを相談すると相手の「助けてあげたい」気持ちが動くことがあるよ。', example: '「最近なんか疲れちゃって笑、〇〇ならこういうときどうする？」' },
      { law: '単純接触効果', title: '返信を求めないスタンプ1個作戦', body: 'スタンプや短い反応だけを送ることで、相手の警戒心を下げながらも存在を気にしてもらえるよ。', example: 'おもしろいスタンプ1個だけ送って、返事を期待しない姿勢で' }
    ],
    usedModel: 'Demo'
  };
  ...
  return {
    ...
    psychology: `今は「話しやすくて好きな人」ポジションにいる感じ。悪い印象はゼロで、${p}とこれから恋愛に発展できる余地はたっぷりあるよ！`,
    advice: `共通の趣味や${p}が好きな話題を見つけて、相手が自分から話してくれるきっかけを作ってみよう。`,
    radar: { intimacy: 55, passion: 52, commitment: 60, status: 48, safety: 62 },
    radarInterpretation: '誠実さと心のゆとりは高め。ときめき・親密性をもう少し上げるとぐっと近づけるかも！',
    matrix: { x: 30, y: -10 },
    matrixInterpretation: '安心・リラックスモード。ちょっとしたサプライズで恋愛モードに切り替えられる！',
    lang: { selfDisclosure: 42, mirroring: 48, pronounCount: 4, emojiSync: 44 },
    langInterpretation: '気を許してるけどまだ恋愛的な意識は薄めかな。',
    approaches: [
      { law: 'ツァイガルニク効果', title: '話を途中で切って「続き」を楽しみにさせる', body: '話を完結させずに「続きは次に会ったとき！」ってすることで、次に会う理由を自然に作れるよ。', example: '「それ気になりすぎる！笑 続きは今度直接話して絶対！」' },
      { law: '吊り橋効果', title: '一緒にちょっとドキドキする体験をしてみる', body: '謎解きや絶叫系など少しスリルがある場所でのデートを提案してみよう！', example: '「謎解きとか好き？笑 一緒に行ってみたいんだけどどう？」' }
    ],
    usedModel: 'Demo'
  };
}
...
function showResult(data, source) {
  ...
  showReportHeader(source || '結果');

  if (data.input) renderInputSummary(data);
  else {
    const sp = $('input-summary-panel');
  ...
  setTimeout(() => {
    const bar = $('report-header-bar');
    const target = bar && !bar.classList.contains('hidden') ? bar : $('result-panel');
...
function renderInputSummary(item) {
  ...
  if (item.isLine) {
    ...
    optionsHtml = `
      <div class="summary-item-row"><span class="summary-item-label">ご相手</span><span class="summary-item-val">${item.partner || '—'}</span></div>
      <div class="summary-item-row"><span class="summary-item-label">相手との関係</span><span class="summary-item-val">${item.rel || '—'}</span></div>
      <div class="summary-item-row"><span class="summary-item-label">会った回数</span><span class="summary-item-val">${meetLabel}</span></div>
      <div class="summary-item-row"><span class="summary-item-label">返信スピード</span><span class="summary-item-val">${speedLabel}</span></div>
      <div class="summary-item-row"><span class="summary-item-label">普段と比べて</span><span class="summary-item-val">${moodStr}</span></div>`;
  } else {
    ...
    optionsHtml = `
      <div class="summary-item-row"><span class="summary-item-label">ご相手</span><span class="summary-item-val">${item.partner || '—'}</span></div>
      <div class="summary-item-row"><span class="summary-item-label">場面・状況</span><span class="summary-item-val">${item.scene || '—'}</span></div>
      <div class="summary-item-row"><span class="summary-item-label">一緒にいた時間</span><span class="summary-item-val">${durationLabel}</span></div>
      <div class="summary-item-row"><span class="summary-item-label">場のテンション</span><span class="summary-item-val">${tensionLabel}</span></div>
      <div class="summary-item-row"><span class="summary-item-label">相手の様子</span><span class="summary-item-val">${attitudeStr}</span></div>`;
  }
  ...
  card.innerHTML = `
    ...
    ${item.extra ? `<div class="summary-section text-section"><div class="summary-text-label">追加情報・背景</div><div class="summary-text-val-extra"></div></div>` : ''}`;

  card.querySelector('.summary-text-val').textContent = item.input || '';
  if (item.extra) card.querySelector('.summary-text-val-extra').textContent = item.extra;
...
function resetView() {
  ...
  if (t === 'line') {
    ...
    if (li) { li.value = ''; }
    const lc = $('line-count');
    if (lc) lc.textContent = '0/50000';
    const lei = $('line-extra-input');
    if (lei) lei.value = '';
    ...
    if (si) si.value = '';
    const sc = $('sit-count');
    if (sc) sc.textContent = '0/50000';
    const sei = $('sit-extra-input');
    if (sei) sei.value = '';
...
function copyCardContent(type) {
  ...
  if (!data) return '';
  let txt = '';
  if (type === 'psych') txt = `◆ 本音の分析\n${data.psychology}`;
  else if (type === 'advice') txt = `◆ 次の一手アドバイス\n${data.advice}`;
  else if (type === 'radar') txt = `◆ 恋愛心理プロファイル💫\n・親密性: ${data.radar.intimacy}\n・ときめき: ${data.radar.passion}\n・誠実さ: ${data.radar.commitment}\n・承認欲求: ${data.radar.status}\n・心のゆとり: ${data.radar.safety}\n解説: ${data.radarInterpretation || ''}`;
  else if (type === 'matrix') txt = `◆ テンション・マトリクス\n座標: ポジ/ネガ ${data.matrix.x > 0 ? '+' : ''}${data.matrix.x} ／ テンション ${data.matrix.y > 0 ? '+' : ''}${data.matrix.y}\n解説: ${data.matrixInterpretation || ''}`;
  else if (type === 'lang') txt = `◆ メッセージ心理分析📊\n・自己開示率: ${data.lang.selfDisclosure}%\n・ミラーリング同調率: ${data.lang.mirroring}%\n・一緒系ワード数: ${data.lang.pronounCount}\n・絵文字シンク率: ${data.lang.emojiSync || 0}%\n解説: ${data.langInterpretation || ''}`;
...
function performCopy(text, buttonEl, isGlobal = false) {
  navigator.clipboard.writeText(text).then(() => {
    if (isGlobal) {
      ...
      setTimeout(() => { buttonEl.innerHTML = originalHTML; buttonEl.classList.remove('copied'); }, 2000);
    }
  });
}
...
async function setShareEnabled(enabled) {
  const isResave = enabled && state.currentResult && !state.currentResult.consultationId;
  const r = await fetch('index.php?action=share-toggle', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      deviceId,
      consultationId: state.currentResult.consultationId || 0,
      enabled,
      record: state.currentResult,
      isResave
    })
  });
  ...
  if (d.newConsultationId) {
    state.currentResult.consultationId = d.newConsultationId;
    // Update in history too
    const idx = state.history.findIndex(h => h.id === state.currentResult.id);
    if (idx >= 0) {
      state.history[idx].consultationId = d.newConsultationId;
...
if (btnShare) btnShare.onclick = function() {
  ...
  if (!state.currentResult.consultationId) {
    // Will do re-save on share toggle/copy
  }
  const shareEnabled = $('share-enabled');
  if (shareEnabled) shareEnabled.checked = !!state.currentResult.shared;
  $('share-modal').classList.add('open');
};

$('share-modal').onclick = e => { if (e.target === $('share-modal')) $('share-modal').classList.remove('open'); };

const shareEnabledEl = $('share-enabled');
if (shareEnabledEl) shareEnabledEl.onchange = async function() {
  if (!state.currentResult) return;
  try {
...
if (btnShareCopy) btnShareCopy.onclick = async function() {
  if (!state.currentResult) return;
  try {
    const d = await setShareEnabled(true);
    const shareEnabled = $('share-enabled');
    if (shareEnabled) shareEnabled.checked = true;
...
function createHistoryItem(item, index) {
  ...
  div.querySelector('.btn-history-delete').onclick = (e) => {
    e.stopPropagation();
...
function renderHistoryAll() {
  ...
  if (el) { el.innerHTML = ''; state.history.forEach((item, index) => el.appendChild(createHistoryItem(item, index))); }
  if (el2) { el2.innerHTML = ''; state.history.forEach((item, index) => el2.appendChild(createHistoryItem(item, index))); }
}
...
if (btnClearEl) btnClearEl.onclick = clearHistory;
const btnClearResult = $('btn-clear-result');
if (btnClearResult) btnClearResult.onclick = clearHistory;
...
/* ─── 初期タブ（?tab=hist等） ─── */
if (window.__initialTab && !window.__isSharedMode) {
  const validTabs = ['line', 'sit', 'hist'];
  if (validTabs.includes(window.__initialTab)) {
    switchTab(window.__initialTab);
  }
}