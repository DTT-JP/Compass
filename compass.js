...
/* ─── チップグループ ─── */
function initCustomChip(groupId, stateKey, inputId, colorClass) {
  const grp = $(groupId);
  const input = $(inputId);
  if (!grp || !input) return;
  ...
  const updateInputUI = () => {
    const useCustom = state[stateKey] && !Array.from(grp.querySelectorAll('.chip')).some(c => c.dataset.val === state[stateKey]);
    input.classList.toggle('hidden', !useCustom && state[stateKey] !== 'その他');
    if (useCustom) input.value = state[stateKey];
  };
...
initCustomChip('partner-group', 'partner', 'partner-custom', 'selected');
initChips('rel-group', 'rel', 'selected');
initMultiChips('mood-group', 'mood', 'selected');
initChips('scene-group', 'scene', 'selected-purple');
initCustomChip('scene-group', 'scene', 'scene-custom', 'selected-purple');
